...
from mcp_core.utils import gitlab_automation as git_service # <--- Switch to GitLab
...
async def process_pipeline(ctx: ToolContext, event: dict, node_id: str, coder: LLMCoder, router_cache: RouterCache, search_engine: RepoSearch, project_root: str) -> bool:
    ...
    try:
        ...
        # Use local temp file -> IN THE PROJECT ROOT so imports work
        ext = ".jsx"  # Prototyping mode: always use JSX
        ...
        try:
            ...
            for attempt in range(2): 
                ...
                logger.info(f"   🛡️ Running Compiler Check (Attempt {attempt+1}) in {project_root}...")
            ...
            # Write back for Prettier
            with open(temp_file, "w", encoding="utf-8") as f:
                f.write(code)
                
            # Try Prettier with timeout (optional - don't block if it fails)
            import subprocess
            try:
                ...
                logger.warning("   ⚠️ Prettier timed out - skipping formatting")
            except Exception as e:
                logger.warning(f"   ⚠️ Prettier failed: {e} - skipping formatting")
        ...
        # 7. Create/Update GitLab Merge Request
        logger.info("   📦 Initiating GitLab MR update...")
...
async def main():
    ...
    # 1. Load Dynamic Configuration (GitLab)
    repo_url = os.getenv("GITLAB_REPO_URL")
    repo_branch = os.getenv("GITLAB_BRANCH", "main")
    sub_dir = os.getenv("REPO_SUB_DIR", "")
    ...
    # The physical path where code lives: ./temp_workspace/{sub_dir}
    # We use this for Validation and Git Context
    ...
    try:
        ...
        # B. Index the code (scoped to sub_dir if set)
    ...
    while True:
        try:
            # Pass project_root to process_tick for validation context
