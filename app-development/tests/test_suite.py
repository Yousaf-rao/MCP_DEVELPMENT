...
class TestFigmaRefinements:
    ...
    def test_soft_fallback_merge(self):
        """Verify CodeMerger falls back to full-file reconciliation if markers are missing."""
        old_code = """
        import React from 'react';
        export const FallbackComponent = () => (
            <div data-mcp-id="1" className="p-4 bg-red-500 hover:bg-red-600">
                <button data-mcp-id="2" className="text-white focus:font-bold">Click</button>
            </div>
        );
        """
        ...
        import React from 'react';
        export const FallbackComponent = () => (
            <div data-mcp-id="1" className="p-8">
                <button data-mcp-id="2" className="text-black">Click</button>
            </div>
        );
        """
        
        # Ensure CodeMerger is available (it should be imported)
        from mcp_core.utils.code_merger import CodeMerger
        
        merged = CodeMerger.merge(old_code, new_code)
        
        # Verify Fallback (Full Overwrite + Patch)
        assert "p-8" in merged
        assert "p-4" not in merged
        assert "hover:bg-red-600" in merged
        # focus:font-bold should be preserved as it is a protected prefix
        assert "focus:font-bold" in merged
        assert "data-mcp-id=\"1\"" in merged
